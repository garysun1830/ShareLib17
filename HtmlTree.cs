...
namespace ShareLib17
{
    public class HtmlTag
    {
        ...
        public string Text
        {
            get
            {
                return HtmlTree.RemoveAllTags(ToString());
            }
        }
    ...
    public class HtmlTree
    {
        ...
        public static List<HtmlTag> FindTags(string Text, string Tag, string pattern, bool ForRegEx)
        {
            if (!ForRegEx)
                return new List<HtmlTag>();
            HtmlTree tree = new HtmlTree(Text, Tag);
            List<HtmlTag> result = tree.FindTags(Tag);
            return result.Where(t => Regex.IsMatch(t.ToString(), pattern, RegexOptions.IgnoreCase)).ToList();
        }
        ...
        public static List<HtmlTag> FindTagsByClass(string Text, string Tag, params string[] Words)
        {
            if (Words == null || Words.Length == 0)
                return null;
            return FindTags(Text, Tag, true, string.Format("class=\"{0}\"", Words[0]), string.Format("class='{0}'", Words[0]));
        }
        ...
        public static HtmlTag FindFirstTag(string Text, string Tag, params string[] Words)
        {
            return FindFirstTag(Text, Tag, true, Words);
        }
        ...
        public static HtmlTag FindFirstTag(string Text, string Tag, string pattern, bool ForRegEx)
        {
            List<HtmlTag> tags = FindTags(Text, Tag, pattern, ForRegEx);
            if (tags.Count == 0)
                return null;
            return tags[0];
        }
        ...
        {
            string result = "";
            Regex rgx = new Regex("href=[\"']?((?!\")(?!').)*[\"']?", RegexOptions.IgnoreCase | RegexOptions.Singleline);
    ...
    public class HtmlLink : HtmlTag
    {
        ...
        public HtmlLink(HtmlTag CopyFrom)
        {
            string s = CopyFrom.ToString();
            Match m = Regex.Match(s, "href=((?! ).)* ", RegexOptions.IgnoreCase);
            if (!m.Success)
                m = Regex.Match(s, "href=((?!>).)*>", RegexOptions.IgnoreCase);
            if (!m.Success)
                return;
            s = m.Value;
            int i = s.IndexOf(">");
            if (i >= 0)
                s = s.Remove(i);
            s = Regex.Replace(s, "href=\"", "", RegexOptions.IgnoreCase).Replace("\"", "").Replace("'", "");
            s = Regex.Replace(s, "href=", "", RegexOptions.IgnoreCase);
            href = s.Trim();
        }
